...
sealed class BoxInfoByNetwork with _$BoxInfoByNetwork {
  ...
  BoxInfo getBoxInfo(VecnoNetwork network) {
    switch (network) {
...
sealed class WalletInfo with _$WalletInfo {
  ...
  bool get hasValidVpub => !kind.isLegacy;

  bool get canSetPassword => !kind.isViewOnly;

  late final String settingsKey = hash('walletSettingsKey#${wid}');

  String hdPublicKey(VecnoNetwork network) {
    if (network == VecnoNetwork.mainnet) {
...
sealed class WalletData with _$WalletData {
  ...
  const factory WalletData.vpub({
    required String name,
    required WalletKind kind,
    required String vpub,
  }) = _WalletDataKpub;
}
