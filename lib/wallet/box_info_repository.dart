...
// Helper to generate strong, unique box keys using SHA-256
String _secureBoxKey(String input) {
  final bytes = utf8.encode(input);
  final digest = sha256.convert(bytes);
  return digest.toString(); // returns 64 hex characters
}
...
BoxInfo _genBoxInfo({
  ...
  // Use distinct inputs for each entity type → no collisions with SHA-256
  final addressBoxKey = _secureBoxKey('address#${wid}#${networkId}');
  final balanceBoxKey  = _secureBoxKey('balance#${wid}#${networkId}');
  final utxoBoxKey     = _secureBoxKey('utxo#${wid}#${networkId}');
  final txIndexBoxKey  = _secureBoxKey('txindex#${wid}#${networkId}');
  final txBoxKey       = _secureBoxKey('tx#${wid}#${networkId}');

  // Optional: log once during development to confirm they are different
  // print('Generated box keys for $wid / $networkId:');
  // print('  address → $addressBoxKey');
  // print('  balance  → $balanceBoxKey');
...
extension BoxInfoExtension on SettingsRepository {
  String _boxInfoBundleKey(String wid) {
    return _secureBoxKey('boxInfoBundleKey#$wid'); // also make this strong if needed
  }
    ...
    if (!bundle.wasMigrated) {
      ...
      bundle = bundle.copyWith(
        byNetworkId: mainnet != null
            ? {
                ...bundle.byNetworkId,
                kVecnoNetworkIdMainnet: mainnet,
              }
            : bundle.byNetworkId,
        wasMigrated: true,
      );
