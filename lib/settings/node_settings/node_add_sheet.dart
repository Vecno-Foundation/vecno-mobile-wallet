    ...
    Future<void> addNode() async {
      ...
      try {
        ...
        try {
          ...
          client = VecnoClient.url(url, isSecure: true);
          nodeInfo = await client.getInfo();
          networkName = (await client.getBlockDagInfo()).networkName;
          isSecure = true;
        } catch (_) {
          // Fallback to insecure connection
          client = VecnoClient.url(url, isSecure: false);
          nodeInfo = await client.getInfo();
          networkName = (await client.getBlockDagInfo()).networkName;
          isSecure = false;
        }

        VecnoNetwork network;
        final parts = networkName.split('-');
        if (parts.length > 1) {
          network = VecnoNetwork.tryParse(parts[1]) ?? networkForPort(port);
        } else {
