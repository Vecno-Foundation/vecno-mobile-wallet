...
const kVecnoDerivationPath = "m/44'/111111'/0'";
...
String convertIfXpub(String hdPubKey) {
  if (hdPubKey.startsWith('xpub')) {
    try {
      ...
      bip32.network = networkTypeForNetwork(VecnoNetwork.mainnet);
...
String convertHdPublicKey(String hdPubKey, VecnoNetwork toNetwork) {
  final network = networkForVpub(hdPubKey);
...
AddressPrefix addressPrefixForNetwork(VecnoNetwork network) {
  switch (network) {
...
class HdWalletViewECDSA extends HdWalletView {
  ...
  HdWalletViewECDSA(String hdPublicKey) {
    final network = networkForVpub(hdPublicKey);
    final networkType = networkTypeForNetwork(network);
...
abstract class HdWallet implements HdWalletView {
  ...
  static String hdPublicKeyFromSeed(
    ...
    final child = bip32.derivePath(kVecnoDerivationPath);
...
class HdWalletEcdsa extends HdWallet {
  ...
  HdWalletEcdsa(Uint8List seed) : super._() {
    _bip32 = BIP32.fromSeed(seed).derivePath(kVecnoDerivationPath);
  }
