...
sealed class Amount with _$Amount {
  ...
  factory Amount.raw(BigInt raw, {TokenInfo tokenInfo = TokenInfo.vecno}) =>
      Amount(raw: raw, tokenInfo: tokenInfo);
  factory Amount.value(
    Decimal value, {
    TokenInfo tokenInfo = TokenInfo.vecno,
  }) => Amount(
    raw: value.shift(tokenInfo.decimals).toBigInt(),
    tokenInfo: tokenInfo,
  );

  factory Amount.rawInt(int raw, {TokenInfo tokenInfo = TokenInfo.vecno}) {
    final rawBigInt = BigInt.from(raw);
    return Amount(raw: rawBigInt, tokenInfo: tokenInfo);
  }

  factory Amount.rawInt64(Int64 raw, {TokenInfo tokenInfo = TokenInfo.vecno}) {
