...
final kVeniPerVecno = BigInt.from(100000000);
final kStorageMassParameter = kVeniPerVecno * BigInt.from(10000);
...
// UnacceptedDAAScore is used for UTXOEntries that were created by transactions in the mempool, or otherwise
// not-yet-accepted transactions.
// Max Uint64 - 18446744073709551615
final kUnacceptedDAAScore = Int64(-1);
...
sealed class Transaction with _$Transaction {
  ...
  RpcTransaction toRpc() {
    return RpcTransaction(
      version: version,
      inputs: inputs.map((input) => input.toRpc()).toList(),
      outputs: outputs.map((output) => output.toRpc()).toList(),
      lockTime: lockTime,
