...
final vecnoPriceProvider = Provider.autoDispose((ref) {
  return ref.watch(coingeckoVecnoPriceProvider);
});
...
final formatedTotalFiatProvider = Provider.autoDispose((ref) {
  final balance = ref.watch(totalBalanceProvider);
  final price = ref.watch(vecnoPriceProvider);
...
final formatedVecnoPriceProvider = Provider.autoDispose((ref) {
  final price = ref.watch(vecnoPriceProvider).price;
  final currency = ref.watch(currencyProvider);
  final symbol = ref.watch(veSymbolProvider);
...
final vecnoFormatterProvider = Provider((ref) {
  final symbol = ref.watch(veSymbolProvider);
  ...
  final formatter = CurrencyFormatter(
    ...
    maxDecimalDigits: TokenInfo.vecno.decimals,
    maxAmount: kMaxVecno,
  );
...
final fiatFormatterProvider = Provider.autoDispose((ref) {
  final price = ref.watch(vecnoPriceProvider);
  final currency = ref.watch(currencyProvider);
  final maxAmount = price.price * kMaxVecno;
