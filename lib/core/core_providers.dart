...
final networkProvider = Provider((ref) {
  final config = ref.watch(vecnoNodeConfigProvider);
...
final _vecnoApiProvider = Provider.autoDispose<VecnoApi>((ref) {
  final apiUrl = ref.watch(vecnoApiUrlProvider);

  if (apiUrl.isEmpty) {
    return VecnoApiEmpty();
  }
  return VecnoApiMainnet(apiUrl);
});

final vecnoApiServiceProvider = Provider.autoDispose<VecnoApiService>((ref) {
  final api = ref.watch(_vecnoApiProvider);
  return VecnoApiService(api);
});

final vecnoClientProvider = Provider((ref) {
  final config = ref.watch(vecnoNodeConfigProvider);
  final inBackground = ref.watch(inBackgroundProvider);

  final client = inBackground
      ? VoidVecnoClient()
      : VecnoClient.url(config.url, isSecure: config.isSecure);
...
final balancesForAddressesProvider = FutureProvider.family
    .autoDispose<Iterable<RpcBalancesByAddressesEntry>, List<String>>(
        (ref, addresses) async {
  final client = ref.watch(vecnoClientProvider);
...
final veSymbolProvider = Provider((ref) {
  final network = ref.watch(networkProvider);

  return switch (network) {
    VecnoNetwork.mainnet => 'VE',
    _ => 'TVE',
  };
});

final symbolProvider = Provider.family<String, Amount>((ref, amount) {
  final veSymbol = ref.watch(veSymbolProvider);
  if (amount.tokenInfo.tokenId != TokenInfo.vecno.tokenId) {
