...
// All new transactions from vecno node
...
final txNotifierForWalletProvider = ChangeNotifierProvider.autoDispose
  ...
  ref.listen(_acceptedTransactionIdsProvider, (_, next) {
    if (next.asData?.value case final ids?) {
      final client = ref.read(vecnoClientProvider);
...
final txConfirmationStatusProvider =
  ...
  final currentBlueScore = ref.watch(virtualSelectedParentBlueScoreProvider);

  final tx = txItem.tx;
  final apiTx = tx.apiTx;
  ...
  final txBlueScore = apiTx.acceptingBlockBlueScore;

  BigInt confirmations = BigInt.zero;

  if (txBlueScore != null) {
    final diff = currentBlueScore - BigInt.from(txBlueScore);
    confirmations = diff < BigInt.zero ? BigInt.zero : diff;
  }

  const kConfirmationThreshold = 100;

  if (confirmations >= BigInt.from(kConfirmationThreshold)) {
