...
final _vecnoPriceRemoteProvider =
  ...
  final cached = ref.read(_vecnoPriceCacheProvider);
  ...
  try {
    ...
    // fallback to Vecnoscan API if CoinGecko API fails
    if (price == null) {
      price = await getVecnoApiPrice(fiat);
    }
    if (price == null) {
    ...
    log.e('Failed to fetch VE exchange rate', error: e, stackTrace: st);
    if (cached.currency == currency.currency) {
      return cached;
    }
    return CoinGeckoPrice(
...
final coingeckoVecnoPriceProvider = Provider.autoDispose((ref) {
  final cache = ref.watch(_vecnoPriceCacheProvider.notifier);
  final remote = ref.watch(_vecnoPriceRemoteProvider);
